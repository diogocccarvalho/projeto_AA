...
def run_presentation():
    ...
    if cerebro_evo: 
        try:
            ...
            print(e)
            ag_evo = AgenteFarolEvo() # Fallback
    else: 
    ...
    ag.learning_mode = False
    sim.adicionar_agente(ag, equipa_id=3, verbose=False)
    # Evo
    if cerebro_evo: 
        try: ag = cerebro_evo.clone()
        except ValueError: ag = AgenteRecolecaoEvo()
    ...
    try:
        for r in range(5):
            ...
            scores = sim.obter_scores_equipas()
            print(f"   Scores -> Equipa Q (1): {scores.get(1,0):.0f} | Equipa Evo (2): {scores.get(2,0):.0f} | Equipa Mista (3): {scores.get(3,0):.0f}")
            
        print("Apresentação terminada.")
        gui.root.destroy()
    except tk.TclError:
...
def main():
    ...
    if MODO == "APRESENTACAO":
        ...
        tasks = [
            ...
            ("TREINO_EVO", "FAROL", "python3"), # Evo usa multiprocess, pypy pode ser instável
