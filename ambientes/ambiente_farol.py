...
class AmbienteFarol(Ambiente):
    ...
    def observacao_para(self, agente):
        ...
        sensores = {}
        dirs = {"Norte":(0,-1), "Sul":(0,1), "Este":(1,0), "Oeste":(-1,0), "NE":(1,-1), "SE":(1,1), "SW":(-1,1), "NW":(-1,-1)}
    ...
    def agir(self, agente, accao):
        ...
        if accao in movs:
            ...
            if (nx, ny) == self.pos_farol: self.terminou = True; return 100.0
            
            reward = -0.2
            if not is_evo: # Shaping apenas para Q-Learning
                reward += (abs(ax-self.pos_farol[0])+abs(ay-self.pos_farol[1]) - (abs(nx-self.pos_farol[0])+abs(ny-self.pos_farol[1]))) * 2.0
            return reward
        return -1.0
